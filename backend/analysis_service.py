...
"""
    ...
    async def generate_strategy_recommendations(
        ...
        # Generate recommendations with Claude (longer timeout due to complex prompt)
        result = await self.claude.generate_analysis(
            rendered_prompt,
            temperature=0.1,  # Lower temperature for more consistent recommendations
            timeout=60  # 60s timeout - strategy recommendations need more time than simple analysis
        )
    ...
    async def generate_strategy_recommendations_stream(
        self,
        strategy: 'InvestmentStrategy',
        ...
        """
        Generate strategy-driven recommendations with streaming (no timeout issues).

        This streaming version yields text chunks as they're generated, preventing
        timeout issues that occur with the standard API call for long responses.

        Now includes caching support - checks cache first and only calls API if needed.
        ...
        Yields:
            Text chunks as they are generated by Claude (or from cache)

        Raises:
            ValueError: If prompt template not found
            Exception: If Claude API streaming fails
        """
        cache_key = f"strategy_recommendations:user_{strategy.user_id}"
        ...
        if not force_refresh:
            ...
            if cached:
                ...
                age = datetime.now(UTC) - generated_at
                if age < timedelta(hours=12):
                    logger.info("Returning cached strategy recommendations (streaming)")
                    # Yield cached JSON as complete response
                    cached_json = json.dumps(cached, default=str)
                    yield cached_json
                    return

        logger.info("Generating fresh strategy recommendations (streaming)")
        ...
        # Accumulate streamed response for caching
        full_response = []

        # Stream recommendations from Claude
        async for chunk in self.claude.generate_analysis_stream(
            rendered_prompt,
            temperature=0.1,  # Lower temperature for more consistent recommendations
            max_tokens=8192,  # Increased from 4096 to ensure complete JSON response
            timeout=120  # Longer timeout for streaming (though won't hit it)
        ):
            full_response.append(chunk)
            yield chunk

        logger.info("Strategy recommendations stream completed")

        # Cache the complete response
        try:
            complete_text = ''.join(full_response)
            recommendations_data = self._extract_json_from_response(complete_text)

            # Store in database for audit trail
            analysis_record = AnalysisResult(
                symbol='PORTFOLIO',  # Special symbol for portfolio-wide analysis
                prompt_id=prompt_template.id,
                prompt_version=prompt_template.version,
                raw_response=complete_text,
                parsed_data=recommendations_data,
                tokens_used=0,  # Not available in streaming mode
                generation_time_ms=0,  # Not tracked in streaming mode
            ...
            # Cache for 12 hours
            await self.cache.set(cache_key, response, ttl=43200)
            logger.info("Cached streamed strategy recommendations for 12 hours")

        except Exception as e:
            logger.error(f"Failed to cache streamed recommendations: {e}")
